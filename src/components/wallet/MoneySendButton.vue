<template>
<div>
  <v-btn block elevation="0" large rounded
         color="primary" class="" 
      ...
      <v-col cols="12" class="px-5">
        ...
        <template v-if="toWalletExistState == true">
          ...
          <div class="pb-4 text-center secondary--text">
            Vous disposez <span class="d-none d-sm-inline">actuellement</span> de <b>{{ $store.getters['auth/amountCurrent'].toFixed(4) }} {{ $store.state.app.monyUnity }}</b>
          </div>

          <v-text-field outlined rounded :value="amoutToSend" 
                        class="toWalletUID mb-2" color="secondary"
                        :rules="[() => (parseFloat(amoutToSend) <= parseFloat($store.getters['auth/amountCurrent']) || amoutToSend == '' || amoutToSend == null) || 'vous ne pouvez pas envoyer plus de ' + $store.getters['auth/amountCurrent'].toFixed(4) + ' òto' ]"
                        placeholder=""  :suffix="$store.state.app.monyUnity"

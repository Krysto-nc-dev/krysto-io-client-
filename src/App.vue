...
import '@/assets/css/nprogress-0.2.0.css'
import '@/assets/css/MaterialDesign-Webfont-master/css/materialdesignicons.min.css'
  ...
  methods: {
    ...
    initSocket : function(){
      ...
      if(socket != null){
        ...
        socket.on('conf-connexion', () => {
          console.log("#WS connected to the socket server")
        ...
        socket.on('new-private-msg', async (data) => {
          ...
          if(this.$route.path == '/private/' + data.newMsg.senderId + '/' + data.offerId)
            ...
            this.$store.dispatch('app/openSnackbar', 
                                  { show: true, color:'blue', 
                                    ...
                                    icon: 'comment',
                                    path: '/private/' + data.newMsg.senderId + '/' + data.offerId })
          }
        })
        ...
        socket.on('deleted-private-msg', async (data) => {
          ...
          if(this.$route.path == '/private/' + data.senderId + '/' + data.offerId)
        ...
        socket.on('new-proposition', async () => {
